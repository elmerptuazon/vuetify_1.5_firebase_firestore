                  ...
                  <v-tooltip bottom nudge-bottom max-width="200">
                    <template v-slot:activator="{ on }">
                      <span v-on="on" class="text-xs-center">
                        <div v-if="
                          props.header.text === 'Edit Variant' || 
                          props.header.text === 'Product Name' ||
                          props.header.text === 'Variant Name'"
                        > {{ props.header.text }}
                        </div>
                        
                        <div v-else v-for="(word, index) in props.header.text.split(' ')" :key="index">
                          {{ word }}
                        </div>
...
export default {
  ...
  data: () => ({
    ...
    headers: [
      {
        text: 'Edit Variant', align: 'center', value: '', sortable: false,
      ...
      {
        text: 'Out of Stock?', align: 'left', value: 'isOutOfStock', sortable: false,
